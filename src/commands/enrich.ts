...
export async function cmdEnrich(opts: {
  ...
  if (!useGithub) {
    ...
    return {
      code: 3,
      errorMessage:
        "GitHub token is required for enrichment (--use-github set)",
    };
  } else {
    try {
      ...
      return {
        code: 3,
        errorMessage: `GitHub enrichment failed: ${errMessage}`,
      };
    }
  }
