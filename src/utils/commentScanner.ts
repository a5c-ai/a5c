...
export function scanMentionsInCodeComments(params: {
  ...
  if (!lang) return []
  if (languageFilters && languageFilters.length && !languageFilters.includes(lang)) return []
  ...
  function collectMentions(snippet: string, whole: string, absIndex: number, file: string, lineHint?: number) {
...
function dedupe(items: Mention[]): Mention[] {
  ...
  for (const m of items) {
    const file = typeof m.location === 'object' && m.location ? m.location.file : ''
