...
async function main() {
  ...
  const token = env('GITLAB_TOKEN', env('A5C_AGENT_GITLAB_TOKEN'));
  const projectPath = params.project || env('GITLAB_PROJECT_PATH', env('GITLAB_REPOSITORY'));
  if (!token || !projectPath) {
    console.error('GITLAB_TOKEN and GITLAB_PROJECT_PATH (or GITLAB_REPOSITORY) must be set.');
    process.exit(1);
  }

  const apiHost = env('GITLAB_HOST', 'https://gitlab.com').replace(/\/$/, '');
  const source = params.head_branch || params.source || env('HEAD_BRANCH', 'a5c/main');
  const target = params.base_branch || params.target || env('BASE_BRANCH', 'main');
  if (!source || !target) {
    console.error('head_branch and base_branch are required.');
    process.exit(1);
  }

  const draftInput = params.PR_DRAFT ?? params.draft;
  const isDraft = draftInput === true || String(draftInput).toLowerCase() === 'true';
  const rawTitle = params.title || params.PR_TITLE || `Merge ${source} into ${target}`;
  const title = isDraft && !/^draft[:\]]/i.test(rawTitle) ? `Draft: ${rawTitle}` : rawTitle;
  const description = params.body || params.PR_BODY || '';
  const labels = uniqueLabels(params.labels || params.PR_LABELS);
  ...
  const base = `${apiHost}/api/v4/projects/${projectId}`;

  // Skip if an open MR already exists for the same source/target
  const listUrl = `${base}/merge_requests?state=opened&source_branch=${encodeURIComponent(source)}&target_branch=${encodeURIComponent(target)}`;
  const existingRes = await fetch(listUrl, { headers });
  if (!existingRes.ok) {
    const text = await existingRes.text();
    console.error(`Failed to list existing merge requests: ${existingRes.status} ${text}`);
    process.exit(1);
  }
  const existing = await existingRes.json();
  if (Array.isArray(existing) && existing.length > 0) {
    console.log(`Merge request for ${source} -> ${target} already exists (IID ${existing[0].iid}).`);
    return;
  }
  ...
  const createRes = await fetch(`${base}/merge_requests`, {
    method: 'POST',
    headers,
  ...
  if (createRes.ok) {
    ...
    console.log(`Created merge request !${body.iid}: ${body.web_url}`);
    return;
  }

  if (createRes.status === 409) {
    console.log(`Merge request for ${source} -> ${target} already exists.`);
    return;
  }

  const errorText = await createRes.text();
  console.error(`Failed to create merge request: ${createRes.status} ${errorText}`);
  process.exit(1);
}
