...
async function main() {
  ...
  const apiUrl = `${host}/api/v4/projects/${encodeURIComponent(parsed.project)}/merge_requests/${parsed.mr_iid}`;
  ...
  if (!response.ok) {
    ...
    console.error(`Failed to close merge request: ${response.status} ${text}`);
    process.exit(1);
  }
}
