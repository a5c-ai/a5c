...
function readMessage(): string {
  if (typeof argv.get("message") === "string")
    return String(argv.get("message"));
  if (typeof argv.get("pr-title") === "string")
    return String(argv.get("pr-title"));
  if (typeof argv.get("file") === "string")
    return fs.readFileSync(String(argv.get("file")), "utf8");
