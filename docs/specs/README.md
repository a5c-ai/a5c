...
Examples: see `docs/examples/enrich.offline.json` (offline; no `enriched` block) and `docs/examples/enrich.online.json` (with minimal `enriched.github`). Both validate against `docs/specs/ne.schema.json`.
