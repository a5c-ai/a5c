...
- Tests: See `test/config.loadConfig.test.ts`, `test/redact.test.ts`, `test/config.precedence.test.ts` and `tests/enrich.redaction.test.ts` for coverage and regression fixtures.
- Large payloads: JSON is read/written from files/stdin/stdout; providers may add streaming in future.

See also: `docs/specs/README.md` and `docs/producer/phases/technical-specs/tech-stack.md`.
