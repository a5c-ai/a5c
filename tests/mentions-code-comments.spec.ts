...
import {
  ...
  parseAddedLinesWithNumbers,
} from "../src/codeComments.js";
...
describe("code comment mentions", () => {
  it("parses added lines with numbers", () => {
    ...
    expect(added[0]).toEqual({
      line: 1,
      text: "// @validator-agent: please review",
    });
  });
  ...
  it("extracts mentions from js/ts comment lines", () => {
    ...
    // check location is set as object
    expect(
      mentions.some(
        (m) =>
          typeof (m.location as any)?.file === "string" &&
          typeof (m.location as any)?.line === "number",
      ),
    ).toBe(true);
  });
});
