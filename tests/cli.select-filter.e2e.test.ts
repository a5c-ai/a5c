...
function runCli(args: string[], expectCode = 0) {
  ...
  if (res.status !== expectCode) {
    throw new Error(`CLI exited with code ${res.status} (expected ${expectCode}):\n${res.stderr}\n${res.stdout}`)
  }
  return res.stdout
}
...
describe('CLI select/filter', () => {
  it('normalize --select type,repo.full_name', () => {
    ...
    const out = runCli(['normalize', '--in', sample, '--select', 'type,repo.full_name'])
  ...
  it('enrich --filter enriched.github.pr.mergeable_state=dirty (may be filtered depending on enrichment)', () => {
    ...
    runCli(['enrich', '--in', sample, '--filter', 'enriched.github.pr.mergeable_state=dirty'], 2)
  })
})
