...
describe("handleEnrich", () => {
  ...
  it("offline mode: attaches stub with reason flag:not_set", async () => {
    const res = await handleEnrich({
      in: "samples/pull_request.synchronize.json",
      labels: [],
      rules: undefined,
  ...
  it("includes patch fields when explicitly enabled (include_patch=true)", async () => {
    ...
    if (Array.isArray(prFiles) && prFiles.length) {
      ...
      const hasPatchProp = prFiles.some((f: any) =>
    ...
    const mockOctokit = {
      repos: {
        async getContent() {
          return {
            data: {
              content: Buffer.from("hello @developer-agent", "utf8").toString(
                "base64",
              ),
              encoding: "base64",
              size: 20,
