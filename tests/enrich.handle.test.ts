...
describe('handleEnrich integrates enrichGithubEvent', () => {
  it('maps PR enrichment under enriched.github and respects include_patch=false', async () => {
    ...
    const mock = {
      pulls: {
        async get() { return { data: { number: 1, state: 'open', merged: false, draft: false, base: { ref: 'main' }, head: { ref: 'feat' }, changed_files: 1, additions: 1, deletions: 0 } } },
        async listFiles() { return { data: [{ filename: 'src/a.js', additions: 1, deletions: 0, changes: 1, patch: '---' }], headers: {}, status: 200 } },
        async listCommits() { return { data: [{ sha: 'abc', commit: { message: 'x' } }], headers: {}, status: 200 } },
      },
      paginate: async (fn: any) => (fn === (mock as any).pulls.listFiles ? [{ filename: 'src/a.js', additions: 1, deletions: 0, changes: 1, patch: '---' }] : [{ sha: 'abc', commit: { message: 'x' } }]),
      repos: {
        async getContent() { throw Object.assign(new Error('no file'), { status: 404 }) },
