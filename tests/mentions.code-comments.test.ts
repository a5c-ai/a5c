...
function mkOctokitWithFiles(
  map: Record<
    string,
    string | { content: string; encoding?: string; size?: number }
  >,
) {
  return {
    repos: {
      async getContent({ path }: { path: string }) {
        ...
        const size =
          val.size ??
          Buffer.byteLength(content, enc === "base64" ? "utf8" : enc);
        const payload =
          enc === "base64"
            ? {
...
describe("mentions extractor - code comments scanning (local content)", () => {
  ...
  it("does not match inside code strings for JS when not in comments", () => {
    ...
    const out = scanMentionsInCodeComments({
      content: codeOnly,
      filename: "a.js",
    });
...
`;
    ...
    const out = await scanCodeCommentsForMentions({
      owner: "o",
      repo: "r",
      ref: "sha",
      files: [{ filename: "src/a.js" }, { filename: "src/b.ts" }],
      octokit,
      options: { languageFilters: ["js", "ts"] },
    });
    const locs = out.map((m) => m.location as any);
    expect(
      locs.some((l) => l?.file === "src/a.js" && typeof l?.line === "number"),
    ).toBe(true);
    expect(
      locs.some((l) => l?.file === "src/b.ts" && typeof l?.line === "number"),
    ).toBe(true);
  ...
  it("finds mentions in README markdown lines", async () => {
    ...
    const out = await scanCodeCommentsForMentions({
      ...
      octokit,
      options: { languageFilters: ["md"] },
    });
    expect(out.length).toBeGreaterThan(0);
    expect((out[0].location as any).file).toBe("README.md");
    expect(typeof (out[0].location as any).line).toBe("number");
  ...
  it("skips large files over size cap", async () => {
    ...
    const out = await scanCodeCommentsForMentions({
      ...
      repo: "r",
      ref: "sha",
      files: [{ filename: "big.js" }],
      octokit,
      options: { fileSizeCapBytes: 200 * 1024, languageFilters: ["js"] },
    });
    expect(out.length).toBe(0);
  });
});
