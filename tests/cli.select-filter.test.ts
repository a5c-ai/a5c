...
import { selectFields, parseFilter, passesFilter } from '../src/utils/selectFilter.js'
...
describe('select/filter integration with handlers', () => {
  it('normalize + select produces only requested fields', async () => {
    const { output } = await handleNormalize({ in: 'samples/push.json', source: 'cli', labels: [] })
  ...
  it('enrich + filter blocks when not matching', async () => {
    const { output } = await handleEnrich({ in: 'samples/pull_request.synchronize.json', labels: [], rules: undefined, flags: {} })
    ...
    const okPresence = passesFilter(output as any, parseFilter('payload.pull_request.mergeable_state'))
