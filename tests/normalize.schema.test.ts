...
describe('normalize → NE schema shape (smoke)', () => {
  it('workflow_run sample produces required fields', async () => {
    const { output } = await handleNormalize({ in: 'samples/workflow_run.completed.json', source: 'cli', labels: ['test'] })
  ...
  it('pull_request sample sets ref base/head and labels preserved', async () => {
    const { output } = await handleNormalize({ in: 'samples/pull_request.synchronize.json', source: 'cli', labels: ['foo=bar'] })
  ...
  it('push sample sets branch ref and sha', async () => {
    const { output } = await handleNormalize({ in: 'samples/push.json', source: 'cli' })
  ...
  it('issue_comment sample sets type and actor', async () => {
    const { output } = await handleNormalize({ in: 'samples/issue_comment.created.json', source: 'cli' })
