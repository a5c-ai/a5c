...
describe('handleEmit', () => {
  it('writes to stdout by default and returns code 0', async () => {
    const tmp = path.join(process.cwd(), 'tests', 'fixtures', 'github', 'push.json')
  ...
  it('writes to file sink when out is provided', async () => {
    const tmpIn = path.join(process.cwd(), 'tests', 'fixtures', 'github', 'push.json')
    const outPath = path.join(process.cwd(), 'tests', 'tmp.emit.out.json')
    try {
      const { code } = await handleEmit({ in: tmpIn, out: outPath, sink: 'file' })
      expect(code).toBe(0)
