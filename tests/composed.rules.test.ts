...
`
    ...
    const samplePath = path.join(process.cwd(), 'samples/pull_request.synchronize.json')
    const sample = JSON.parse(fs.readFileSync(samplePath, 'utf8'))
    sample.pull_request.mergeable_state = 'dirty'
    sample.pull_request.labels = (sample.pull_request.labels || []).concat([{ name: 'priority:low' }])

    const tmpEvent = path.join(tmpDir, 'event.json')
    fs.writeFileSync(tmpEvent, JSON.stringify(sample), 'utf8')

    const { output } = await handleEnrich({ in: tmpEvent, rules: rulesFile, flags: {} })
    ...
    const item = composed.find((c: any) => c.key === 'conflict_in_pr_with_low_priority_label')
    ...
    expect(typeof item.reason === 'string' || item.reason === undefined).toBeTruthy()
  })
})
