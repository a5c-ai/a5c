...
function runDetector(xml: string) {
  const src = fs.readFileSync(path.resolve('scripts/flaky-detector.cjs'), 'utf8')
  // crude way: eval the module functions by writing a temp file and requiring
  const tmp = path.resolve('.tmp-flaky.js')
  fs.writeFileSync(tmp, src)
  // Shim process to capture stdout
  const { spawnSync } = require('child_process')
  const junit = path.resolve('.tmp-junit.xml')
  fs.writeFileSync(junit, xml)
  const res = spawnSync('node', [tmp], { env: { ...process.env, JUNIT_XML: junit }, encoding: 'utf8' })
  fs.unlinkSync(tmp)
  fs.unlinkSync(junit)
  const out = res.stdout?.trim() || '{}'
  try { return JSON.parse(out) } catch { return { flakies: [], found: false } }
}
