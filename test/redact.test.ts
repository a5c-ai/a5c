...
describe("redaction", () => {
  ...
  it("masks JWT in string", () => {
    const got = redactString(
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjMifQ.sgn",
    );
  ...
  it("masks Slack/AWS/Basic Auth patterns in strings", () => {
    const input =
      "xoxb-1234567890-abcdefghijkl https://user:secret@example.com AKIA1234567890ABCD";
  ...
  it("masks representative fixture payload values", () => {
    const json = JSON.parse(
      require("node:fs").readFileSync(
        "tests/fixtures/redaction/sample-with-secrets.json",
        "utf8",
      ),
    );
