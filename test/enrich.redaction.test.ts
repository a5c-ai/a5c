...
describe('enrich output redaction', () => {
  it('masks token-like strings in enriched metadata and fields', async () => {
    const { output } = await handleEnrich({ in: 'samples/issue_comment.created.json', labels: [], rules: undefined, flags: {} })
    ...
    const withSecrets: any = {
      ...
      enriched: {
        ...
        metadata: { ...(output.enriched as any)?.metadata, bearer: 'Bearer abcdefghijklmnop123456' },
        derived: { ...(output.enriched as any)?.derived, token_hint: 'ghp_1234567890abcdef1234567890abcdef1234' },
      },
    }
